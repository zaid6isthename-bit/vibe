...
export default function PersistentBackground() {
  ...
  const hueShift = equippedTheme ? 0 : THEME_PRISM[activeTheme] ?? 0;
    ...
    <div className="fixed inset-0 z-0 overflow-hidden bg-[#05070A] pointer-events-none">
      ...
      <div className="absolute inset-0 z-[2] opacity-40">
        <LightRays
          ...
          raysColor={equippedTheme ? "#ffffff" : "#71ffe8"}
          raysSpeed={0.6}
          lightSpread={0.55}
          rayLength={2.6}
          followMouse
        ...
        <AnimatePresence mode="wait">
          {activeBackdrop === "sanctuary-backdrop" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 30% 70%, rgba(34,197,94,0.18) 0%, transparent 60%)",
              }}
            />
          )}
          {activeBackdrop === "neural-flow-backdrop" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 20% 20%, rgba(99,102,241,0.28) 0%, rgba(20,184,166,0.18) 50%, transparent 70%)",
              }}
            />
          )}
          {activeBackdrop === "obsidian-void-backdrop" && (
            <motion.div
              key="backdrop-obsidian"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
        ...
        <AnimatePresence mode="wait">
          {activeTheme === "zen-sanctuary" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 50% 100%, rgba(74,222,128,0.12) 0%, transparent 60%)",
              }}
            />
          )}
          {activeTheme === "neural-cinematic" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 50% 50%, rgba(113,255,232,0.10) 0%, rgba(0,209,255,0.05) 50%, transparent 70%)",
              }}
            />
          )}
          ...
          {activeTheme === "editorial-silk" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 20% 80%, rgba(241,201,125,0.12) 0%, transparent 55%)",
              }}
            />
          )}
          {activeTheme === "oracle-architect" && (
            <motion.div
              ...
              style={{
                background:
                  "radial-gradient(ellipse at 50% 0%, rgba(0,242,255,0.15) 0%, transparent 40%)",
              }}
            />
          )}
        </AnimatePresence>
